...
// when printing, if the character is unknown, we print the
// character for pi instead.
...
// there should be a constants crate that provides these
// machine constants typing as `u64` basically because i set
// pointer width to 64 (which probably doesn't matter in the
...
impl Screen {
    ...
    fn clear(&mut self) {
        // might be better to have a blank screen lying around that
        // we could just assign here? instead of copying the
        // blank row everywhere.
    ...
    fn print_sc(&mut self, sc: ScreenChar) {
        ...
        // `write_volatile` prevents any compiler optimization due
        // to write but no reads. here's we're using the
        // read/write semantics to communicate
        // to a peripheral device. alors, the compiler shouldn't
        // optimize the operation away. writing volatile-ly
        // blocks the potential removal.
        //
    ...
    fn print(&mut self, c: u8) {
        // if we're at the end of the buffer's width, we go to the
        // next line before we print.
        if self.should_wrap() {
            self.print_new_line();
        }
    ...
    fn print_new_line(&mut self) {
        match self.tfd {
            ...
            TextFlowDirection::TopDown => {
                // TODO
                // we only have to worry about a complicated
                // scroll when we've filled
                // the entire screen. then we can repeat the
                // new line operation for the
    ...
    pub fn print_text(&mut self, text: &str) {
        ...
        // emojis aren't included. we'd have to put the adapter in a
        // different mode to enable drawing and displaying
        // emojis.
